...
void startDrawing(vita2d_texture *bg) {
  ...
  vita2d_set_clear_color(VTB_BACKGROUND_COLOR);
  vita2d_clear_screen();
...
void startDrawing2() {
	vita2d_start_drawing();
	vita2d_set_clear_color(BOOK_IMAGES_MENU_COLOR);
	vita2d_clear_screen();	
}
...
int debugPrintf(const char *text, ...) {
  ...
  SceUID fd = sceIoOpen("ux0:data/VitaTweakBox_log.txt", SCE_O_WRONLY | SCE_O_CREAT | SCE_O_APPEND, 0777);
  if (fd >= 0) {
